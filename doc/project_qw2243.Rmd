...
setwd("/Users/whs/Documents/GitHub/Spring2018-Project1-qw2243c/data")
```
How the inaugural speeches of U.S. presidents reveal the underlying history background?
how indicative their topics is to reveal the current situation of America

```{r, message=FALSE, warning=FALSE}
###install packages needed
packages.used=c("rvest", "tibble", "qdap", "gplots", "dplyr","gcookbook",
...
Given that the inaugural speech represents the very begining of the president's term of office and delivers to the whole nations, most presidents tended to reiterate and emphasize the core values related to the country and discussed the topics related to "government", "states" and "people". In addition, they expressed their determinations and willingness in solving the issues people were concerned about, by using the words like "will", "shall" and "must". The wordcloud reveals 100 words, most of which are the most frequently used words in public speeches. 


Next, I'm interested in the concentration and emphasis of individual president's speeches over different periods of the history and the connections with the significant historical events. I chose the five most representative prisidents, George Washington (1789-1797), Abraham Lincoln(1861–1865), Franklin D. Roosevelt(1933–1945), John F. Kennedy(1961–1963), George W. Bush(2001-2009) from the list and analyze their wordcloud seperately. 
...
wordcloud(ff.dtm.1$term[ff.dtm.1$document==1],ff.dtm.1$count[ff.dtm.1$document==1],
          scale=c(3 ,0.4), min.freq=1, max.words= 80, 
          random.order=FALSE, random.color = FALSE,
          rot.per=0.2, colors=brewer.pal(4, "OrRd"))
### President Abraham Lincoln
wordcloud(ff.dtm.1$term[ff.dtm.1$document==19],ff.dtm.1$count[ff.dtm.1$document==19],
          scale=c(3 ,0.4), min.freq=1, max.words= 80, 
          random.order=FALSE, random.color = FALSE,
...
wordcloud(ff.dtm.1$term[ff.dtm.1$document==44],ff.dtm.1$count[ff.dtm.1$document==44],
...
```
If I compare the results from the two speeches from the very first prrsident George Washington, the most commonly used words are "providential", "immutable", "arrive". This is consistent with the then historical background that Washington, as the founder of the new established country, illustraed his political philosophy of seperation of powers and the formulation of a unifying Constitution. Lacking precedents to guide, Washington called to attention of the "pure and immutable principles" and wish the country to thrive. 


Next, if we look into President Abraham Lincoln's speeches, the most commonly used words are "minority", "secede", "surrendered", "offenses" and "slaves". President Lincoln was regarded as one of the most honorable president because he led the United States through its Civil War. In that bloodiest era, he not only preserved the Union and strengthened the federal government, but also paved the way for the abolition of slavery. The words in the WordCloud are obviously related to the topics of slavery and Civil War. 
...
Those individual speeches shows that the history, in another words, the domestic and international situation has an impact on president's inaugural speeches and the following decision-making in his term. In turn, president's decision-making would also influence the history.


Furthermore, I would also like to see if the contents from two different presidents would vary a lot given they are from totally different historical backgrounds. 
